...
**Live demo:** [rag-image-captioning-fxxzqkhwjuqyx5s6rthwxj.streamlit.app](https://rag-image-captioning-fxxzqkhwjuqyx5s6rthwxj.streamlit.app/), deployed via Streamlit Community Cloud from this repo, main file `demo/demo_app.py`. It opens on a **precomputed gallery**, not an upload box: 40 held-out COCO val2017 images run offline through all 8 configs this project's ablation covers (2 selection strategies x 4 crop configs, via `scripts/evaluate.py --save-predictions` + `scripts/build_gallery_data.py`), so flipping the selection/crop toggles shows the real best-scoring config's actual output — including medoid + segmentation crops — at zero runtime cost. A "Upload your own" second tab still runs live retrieval, honestly scoped to what it actually is: Community Cloud's free tier can't run the Docker-based full app, so that path is retrieval-only, no crops, no BLIP toggle. The demo index (COCO val2017 + Indian-context images) for the upload tab is hosted separately as a public HF Dataset ([AntiVicious/rag-image-captioning144-index](https://huggingface.co/datasets/AntiVicious/rag-image-captioning144-index)) and downloaded on first boot. `demo/` has its own, deliberately unpinned `requirements.txt` and deliberately excludes `transformers` entirely (not just DETR/BLIP) — the root `requirements.txt`'s exact pins (chosen for real, documented bugs on the local dev machine — see below) turned out to transitively drag in build failures against whatever Python Community Cloud's free tier happens to default to, on three different packages before this was isolated. The real shipped default (medoid + segmentation crops) needs `transformers`/DETR and more RAM than the free host offers for live inference — run this repo locally with Docker for that, or see the gallery tab for its real output.
